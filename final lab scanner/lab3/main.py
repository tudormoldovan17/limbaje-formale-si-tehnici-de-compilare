...
class Scanner:
    ...
    def read_tokens(self):
        with open("token.in", "r") as token_file:
            for line in token_file:
                ...
                if token in ["arry", "in", "out", "iff", "elsee", "woah", "loop", "tru", "fls", "cnst"]:
    ...
    def treat_string_constant(self):
        ...
        position = (self.current_line, position)
    ...
    def check_if_valid(self, possible_identifier):
        if possible_identifier in self.reserved_words:
            return False
        if re.search(r'\b(dec|flo|chr|bol)\b', possible_identifier):
            self.ok = 1
            return False
        if self.ok == 1 and re.search(r'^[A-Za-z][A-Za-z0-9_]*', possible_identifier):
            self.ok = 0
            return True
    ...
    def treat_identifier(self):
        ...
        position = (self.current_line, symbol_table_position)
